...
> **Version:** 4.1.0 (guaraná) — 2026-03-04  
...
| Fraud simulation | 4 transaction types, 4 ride-share types, configurable rate |
...
**Fraud types:**
- `pix_cloning` — PIX key hijacking simulation
- `card_takeover` — credential theft + foreign IP
- `social_engineering` — unusually large transfers, new beneficiary
- `account_takeover` — login from new device, location anomaly
...
**Fraud types:**
- `gps_spoofing` — route distance / fare mismatch
- `fake_ride` — completion reported with zero actual movement
- `driver_collusion` — coordinated route inflation
- `payment_fraud` — legitimate ride, fraudulent payment method
...
*Generated by GitHub Copilot — Brazilian Fraud Data Generator v4.1.0*
